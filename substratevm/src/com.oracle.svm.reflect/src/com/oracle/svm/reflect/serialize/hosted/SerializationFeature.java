...
final class SerializationBuilder {
    ...
    private final Method generateConstructor;
    ...
    SerializationBuilder(FeatureImpl.DuringSetupAccessImpl access) {
        try {
            ...
            generateConstructor = ReflectionUtil.lookupMethod(reflectionFactoryClass, "generateConstructor", Class.class, Constructor.class);
    ...
    private Constructor<?> newConstructorForSerialization(Class<?> serializationTargetClass, Constructor<?> customConstructorToCall) {
        try {
            if (customConstructorToCall == null) {
                ...
                /*
                 * Be cause of the early exit in
                 * ReflectionFactory.newConstructorForSerialization(Class<?>, Constructor<?>) we
                 * have to use ReflectionFactory.generateConstructor(Class<?>, Constructor<?>)
                 * instead so that we can rely on getConstructorAccessor to always return non-null
                 * values for constructors generated by this method.
                 */
                return (Constructor<?>) generateConstructor.invoke(reflectionFactory, serializationTargetClass, customConstructorToCall);
            }
        } catch (ReflectiveOperationException e) {
            throw VMError.shouldNotReachHere(e);
        }
